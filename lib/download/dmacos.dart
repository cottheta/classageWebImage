...
class Dmacos extends StatefulWidget {
  const Dmacos({Key key}) : super(key: key);

  @override
  _DmacosState createState() => _DmacosState();
}

class _DmacosState extends State<Dmacos> {
  @override
  Widget build(BuildContext context) {
    return MyLayout(
      tabletView: macTablet(context),
      desktopView: macDesktop(context),
    );
  }
}

Widget macDesktop(BuildContext context){
  return Scaffold(
...
Widget macTablet(BuildContext context){
  return Scaffold(
    body: SingleChildScrollView(
      child: Column(
        children: <Widget>[
...
Widget dMacContainer(BuildContext context) {
  return Container(
    width: 600,
